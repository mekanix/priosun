...
use priosun::{bhyve, config, jail, output, protocol, util};
use std::io::Write;
use tabled::Tabled;
...
fn main() -> Result<()> {
    ...
    match cli.command {
        Commands::Create { resource } => match resource {
            ...
            CreateResource::Jail { name, set, base } => {
                let config = config::Config::load()?;
                let (_, transcript) = util::cmd::capture(|| {
                    jail::create(&name, set.as_deref(), false, base.as_deref(), &config)
                })?;
                print_transcript(&transcript)?;
            }
            CreateResource::Base { name, set } => {
                let config = config::Config::load()?;
                let (_, transcript) = util::cmd::capture(|| {
                    jail::create(&name, set.as_deref(), true, None, &config)
                })?;
                print_transcript(&transcript)?;
            }
            CreateResource::Vm {
        ...
        Commands::Destroy { resource, name } => {
            let config = config::Config::load()?;
            match resource {
                DestroyResource::Vm => bhyve::destroy(&name, &config)?,
                DestroyResource::Jail => jail::destroy(&name, &config)?,
                DestroyResource::Base => jail::destroy_base(&name, &config)?,
                DestroyResource::Dataset | DestroyResource::Volume => {
        ...
        Commands::Login { name: _ } => {
            bail!("login is handled by priosund");
        }
